    ...
    const $ = {
      ...
      datGui() {
        ...
        const params = {
          ...
          rotation: () => {
            // 绕y轴旋转半周
            // this.mesh.rotation.y += Math.PI; 如果这样写会立即旋转
            // 所以下面用gsap这个库 让库内部逐渐去修改这个值。（这也是动画库的本质：内部逐渐多次修改这个值）
        ...
        gui
          .add(this.mesh.geometry.parameters, "widthSegments", 3, 100, 1)
          .onChange((val) => {
            params.widthSegments = val;
            params.generateGeometry();
          });
        gui
          .add(this.mesh.geometry.parameters, "heightSegments", 3, 100, 1)
          .onChange((val) => {
