  ...
  useEffect(() => {
    const $ = {
      ...
      loadTextures(){
        ...
        const hdrTexture = hdrLoader.load('/public/textures/rectangular/san_giuseppe_bridge_2k.hdr',()=>{
      ...
      createObjects() {
        ...
        const mesh = new THREE.Mesh(new THREE.SphereGeometry(1,64,16),new THREE.MeshPhysicalMaterial({
          // map:this.texture,
          // envMap:this.hdrTexture,
          envMapIntensity:1,
          // metalnessMap:this.metalTexture,
      ...
      datGui(){
        ...
        gui.add(this.mesh.material,'envMap',['hdr','cube']).onChange(val=>{
          if(val === 'cube'){
        ...
        gui.add({
          exposure:1
        },'exposure',0,10,0.1).onChange(val=>{
          // this.renderer.toneMapping = THREE.LinearToneMapping
          this.renderer.toneMappingExposure = val
        })

        gui.add(this.renderer,
            'toneMapping',
            ['NoToneMapping',
            'LinearToneMapping',
            'ReinhardToneMapping',
            'CineonToneMapping',
            'ACESFilmicToneMapping']).onChange(val=>{
